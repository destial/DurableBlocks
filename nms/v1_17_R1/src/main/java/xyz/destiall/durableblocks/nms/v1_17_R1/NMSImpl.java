...
public class NMSImpl implements NMS {
    ...
    public void sendBreakingAnimation(Player player, DurableBlock block) {
        ...
        DedicatedPlayerList server = ((CraftWorld) location.getWorld()).getHandle()
                .getCraftServer().getHandle();
        if (player == null) {
            server.sendAll(packet, ((CraftWorld)location.getWorld()).getHandle());
            return;
        }
        server.sendPacketNearby(((CraftPlayer) player).getHandle(),
            location.getX(), location.getY(), location.getZ(),
            60, ((CraftWorld) player.getWorld()).getHandle().getDimensionKey(), packet);
    }
    ...
    public void clearBreakingAnimation(DurableBlock block) {
        ...
        PacketPlayOutBlockBreakAnimation packet =
                new PacketPlayOutBlockBreakAnimation(
                        ...
                        block.getStage());
        ((CraftWorld) location.getWorld()).getHandle().getCraftServer().getHandle()
